...
impl<T: ByteBufferRead> ByteBufferRead for Wrapping<T> {
    #[inline]
    fn read_from_buffer(buffer: &mut ByteBuffer) -> Result<Wrapping<T>> {
        Ok(Wrapping(buffer.read::<T>()?))
    }

    #[inline]
    fn read_from_buffer_le(buffer: &mut ByteBuffer) -> Result<Wrapping<T>> {
        Ok(Wrapping(buffer.read_le::<T>()?))
    }

    #[inline]
    fn read_from_buffer_be(buffer: &mut ByteBuffer) -> Result<Wrapping<T>> {
...
impl<T: ByteBufferRead> ByteBufferRead for Saturating<T> {
    #[inline]
    fn read_from_buffer(buffer: &mut ByteBuffer) -> Result<Saturating<T>> {
        Ok(Saturating(buffer.read::<T>()?))
    }

    #[inline]
    fn read_from_buffer_le(buffer: &mut ByteBuffer) -> Result<Saturating<T>> {
        Ok(Saturating(buffer.read_le::<T>()?))
    }

    #[inline]
    fn read_from_buffer_be(buffer: &mut ByteBuffer) -> Result<Saturating<T>> {
